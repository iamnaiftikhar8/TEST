...
def generate_complete_analysis_pdf(analysis_data: Dict[str, Any]) -> bytes:
    ...
    try:
        print("🔄 Starting COMPLETE PDF generation with FPDF...")
        ...
        pdf.cell(0, 8, f"Total Rows: {profiling.get('rows', 'N/A')}", 0, 1)
        pdf.cell(0, 8, f"Total Columns: {profiling.get('columns', 'N/A')}", 0, 1)
        pdf.cell(0, 8, f"Missing Values: {profiling.get('missing_total', 'N/A')}", 0, 1)
        pdf.cell(0, 8, f"Numeric Columns: {len(profiling.get('numeric_columns', []))}", 0, 1)
        pdf.ln(10)
        ...
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 8, f"Total Rows: {kpis.get('total_rows', 'N/A')}", 0, 1)
        pdf.cell(0, 8, f"Total Columns: {kpis.get('total_columns', 'N/A')}", 0, 1)
        pdf.cell(0, 8, f"Missing Data: {kpis.get('missing_pct', 'N/A')}%", 0, 1)
        pdf.cell(0, 8, f"Duplicate Rows: {kpis.get('duplicate_rows', 'N/A')}", 0, 1)
        pdf.cell(0, 8, f"Statistical Outliers: {kpis.get('outliers_total', 'N/A')}", 0, 1)
        
        if kpis.get('rows_per_day'):
            pdf.cell(0, 8, f"Rows Per Day: {kpis.get('rows_per_day')}", 0, 1)
